...
* @Date:   2018-04-30 16:08:11
* @Last Modified by:   huanghanzhilian
* @Last Modified time: 2018-04-30 18:50:28
*/
var mm = require("util/mm.js");
var orderAddressService = require("service/address-service.js");
...
var page = {
    ...
    addAddress: function() {
        ...
        if (validated.isOk) {
            // 进行请求
            console.log("验证通过");
            orderAddressService.save(info, function(res) {
                //    执行成功的回调函数
                console.log("添加地址");
    ...
    getAddressInfo: function() {
        ...
        if (this.option.isUpdate) {
            this.data.newAddressInfo.id = $("#receiver-id").val();
        };
    ...
    validateForm: function(addressInfo) {
        ...
        // 验证姓名
        if (mm.validate(addressInfo.receiverName, "require")) {
            status.isOk = true;
        ...
        // 验证地址
        if (mm.validate(addressInfo.receiverAddress, "require")) {
            status.isOk = true;
        } else {
            status.isOk = false;
            status.msg = "请填写收件人地址";
        ...
        // 验证号码
        if (mm.validate(addressInfo.receiverPhone, "require")) {
